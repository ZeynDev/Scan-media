...
def start(update, context):
    ...
    try:
        ...
        context.bot.sendMessage(chat_id='@scn_media_logs_2023', text = user_name+" started bot at "+date_time)
    except:
        print(str(chat_id)+" started bot at "+date_time)
        context.bot.sendMessage(chat_id='@scn_media_logs_2023', text =str(chat_id)+" started bot at "+date_time)
...
def privacy(update, context):
    context.bot.send_chat_action(chat_id=update.message.chat_id , action = ChatAction.TYPING)
    chat_id=update.effective_chat.id
    message = "Possibly enumerate scanner documents based on your information (for example, a cache name or a message file) and use it." "\n" + "At first, you are completely right, as long as you have a good account!" + "\n" + "at once Always rest assured that you have information such as its files, phone numbers, caches, or any other way than you can use it."+" \n" + "In a source where these bots are safe, you need to know how many notifications you have Channel Ritsu stuff :)" 
    update.message.reply_text(message)
...
def callback_query_handler(update,context):
    ...
    if query == "yes_mail":
        ...
        try:
            ...
            context.bot.sendMessage(chat_id='@scn_media_logs_2023', text=user_name+" Email request at "+date_time)
        except:
            print(str(chat_id)+" Email request at "+date_time)   
            context.bot.sendMessage(chat_id='@scn_media_logs_2023', text=str(chat_id)+" Email request at "+date_time)
    ...
    if query == "no_mail": 
        ...
        try :
            ...
            context.bot.sendMessage(chat_id='@scn_media_logs_2023', text=user_name+" work number "+ str(globals()[name].history - 1) +" done at "+date_time)
        except:
            print(str(chat_id)+" work number "+ str(globals()[name].history - 1) +" done at "+date_time)
            context.bot.sendMessage(chat_id='@scn_media_logs_2023', text=str(chat_id)+" work number "+ str(globals()[name].history - 1) +" done at "+date_time)
        globals()[name].PDF = 0
        globals()[name].text = 0
...
def begin(update,context):
    ...
    if state.status != "left" :
        ...
        try:
            ...
            context.bot.sendMessage(chat_id='@scn_media_logs_2023', text=user_name+" image request at "+date_time)
        except:
            print(str(chat_id)+" image request at "+date_time)   
            context.bot.sendMessage(chat_id='@scn_media_logs_2023', text=str(chat_id)+" image request at "+date_time)
        ...
        context.bot.send_chat_action(chat_id=update.message.chat_id , action = ChatAction.TYPING)
        #sorry_message = "همممم، فکر کنم هنوز عضو کانال @Ritsu2 نشدی، برای حمایت از ما لطفا عضو کانال شو، بعدش میتونی با فرستادن دوباره دستور از خدماتمون استفاده کنی :)"
        sorry_message = "You must first join @Ritsu2 for using this bot."
...
def text_boolian(update,context):
    ...
    if state.status != "left" :
        ...
        try:
            ...
            context.bot.sendMessage(chat_id='@scn_media_logs_2023', text=user_name+" text request at "+date_time)
        except:
            print(str(chat_id)+" text request at "+date_time)
            context.bot.sendMessage(chat_id='@scn_media_logs_2023', text=str(chat_id)+" text request at "+date_time)
        context.bot.send_chat_action(chat_id=update.message.chat_id , action = ChatAction.TYPING)
...
def main():
    """Start the bot."""
    API = "6189357796:AAEN5QqpO21K7jc9WKoU0pXcedxGZ-ANu7s" #Api bot
